...
export function nativeManifestPath(platform = process.platform, home = dshHome()) {
    if (platform === 'darwin') {
...
export function extensionInstallDir(home = dshHome()) {
    return join(home, 'chrome-extension', 'dsh-chrome');
}
export function nmBridgeConfigPath(home = dshHome()) {
    return join(home, 'nm-bridge.json');
}
export function launcherPath(home = dshHome()) {
...
export function readBridgeToken(home = dshHome()) {
    try {
...
export function runSetup(opts) {
    ...
    writeFileSync(join(extDir, 'bridge_config.js'), `globalThis.DSH_BRIDGE_CONFIG = ${JSON.stringify(bridgeConfig)};\n`, 'utf8');
...
export function installStatus(home = dshHome()) {
    const extDir = extensionInstallDir(home);
    let wsUrl;
    try {
