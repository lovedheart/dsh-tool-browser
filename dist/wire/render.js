...
import { renderErrorText } from "../governance/teaching.js";
import { BrowserError } from "../governance/errors.js";
const TRUNCATION_FOOTER = (file) => file
 ...
 */
export function renderExecResult(result, maxOutputChars, spillDir) {
    let text;
    if (result.error) {
        const err = new BrowserError({
            category: result.error.category ?? 'INTERNAL',
            cause: result.error.cause ?? 'internal',
