...
const COMMAND_RECEIPT_PREFIX = "dshCommandReceipt:";
const COMMAND_EVICTIONS_KEY = "dshCommandReceiptEvictions";
const COMMAND_RECEIPT_TTL_MS = 5 * 60 * 1000;
const COMMAND_RECEIPT_CAPACITY = 256;
const LAST_DISCONNECT_KEY = "dshLastDisconnect";
...
const bridgeConfig = globalThis.DSH_BRIDGE_CONFIG || {};
const extensionBuild = bridgeConfig.build || {};
const LOCAL_DSH_PORTS = new Set([
...
async function restoreManagedTabs() {
  ...
  disconnectedSince =
    Number.isFinite(restoredDisconnectedSince) && restoredDisconnectedSince > 0
      ...
      : null;
  const restoredLastDisconnect = data["dshLastDisconnect"];
  if (
...
async function storeTabProtocolMetadata(tabId, metadata) {
  ...
  const normalized = {
    ...
    createdByDsh: Boolean(metadata.createdByDsh),
  };
...
function tabLifecycleEventParams(tab, extra) {
  ...
  if (tabId !== undefined) {
    ...
    params.createdByDsh = createdTabs.has(tabId);
...
function isProtectedTabUrl(url) {
  ...
  return (
    LOCAL_DSH_HOSTS.has(parsed.hostname) &&
    LOCAL_DSH_PORTS.has(parsed.port)
  );
}
  ...
  return Promise.all(
    visibleTabs.map(async (tab) => {
      ...
      return {
        ...
        createdByDsh:
          metadata.createdByDsh !== undefined
            ? metadata.createdByDsh
            : tab && tab.id !== undefined
...
async function openExtensionsManager() {
  ...
  // user-owned Chrome page, not a DSH-controlled browser session tab.
  const tab = await chrome.tabs.create({
  ...
  try {
    switch (message && message.method) {
      ...
      case "file.upload":
        return jsonRpcResult(id, {
          ok: false, error_code: "capability_missing",
          message: "File upload is handled by the DSH Browser connection.",
        });
      case "download.read":
        return jsonRpcResult(id, {
          ok: false,
...
function isLocalDshExternalOrigin(origin) {
  try {
...
function handleExternalMessage(message, sender, sendResponse) {
  if (!isLocalDshExternalOrigin(externalMessageOrigin(sender))) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message && message.source === "dsh-chrome-popup") {
