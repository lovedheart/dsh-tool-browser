...
export function renderErrorText(error: BrowserError, stdout = ''): string {
  ...
  if (error.suggested_action) parts.push(error.suggested_action);
...
export function toBrowserError(err: unknown): BrowserError {
  ...
  if (/is not defined/.test(message)) {
    return new BrowserError({
      ...
      reason: message,
      suggested_action:
        'The Browser SDK is already in scope as `Browser`. Begin with ' +
  ...
  if (/strict mode violation/i.test(message)) {
    return new BrowserError({
      ...
      suggested_action:
        'Narrow the locator: add a name/text filter, use .first/.nth(i), or a more ' +
