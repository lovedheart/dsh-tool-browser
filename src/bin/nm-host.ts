...
function writeNmFrame(payload: Buffer): Promise<void> {
  if (payload.length > NM_MAX_OUTBOUND_BYTES) {
    return Promise.reject(
      new Error(
        `core message is ${payload.length} bytes, above Chrome's ${NM_MAX_OUTBOUND_BYTES}-byte host-to-extension limit`,
      ),
    );
  }
...
function readOneFrameSync(): Buffer | null {
  const head = Buffer.alloc(4);
  let off = 0;
  while (off < 4) {
    const got = readSync(0, head, off, 4 - off, null);
    if (got === 0) return off === 0 ? null : (() => { throw new Error('Incomplete NM length prefix'); })();
    off += got;
  }
  const size = head.readUInt32LE(0);
  if (size > NM_MAX_INBOUND_BYTES) throw new Error(`inbound frame ${size} exceeds limit`);
  const payload = Buffer.alloc(size);
  off = 0;
  while (off < size) {
    const got = readSync(0, payload, off, size - off, null);
    if (got === 0) throw new Error('Incomplete NM payload');
    off += got;
  }
  return payload;
}
...
function loadConfig(): { wsUrl: string; token: string } {
  const cfgPath = join(dshHome(), 'nm-bridge.json');
  const raw = JSON.parse(readFileSync(cfgPath, 'utf8'));
  const wsUrl = String(raw.ws_url ?? '').trim();
  const token = String(raw.token ?? '').trim();
  if (!wsUrl) throw new Error('NM bridge ws_url is required');
  if (!token) throw new Error('NM bridge token is required');
  return { wsUrl, token };
}
...
function waitHelloAck(ws: WebSocket, timeoutMs = 5000): Promise<void> {
 ...
 */
async function pumpStdin(wsHolder: { ws: WebSocket | null }): Promise<void> {
  const fd = 0;
  let buf = Buffer.alloc(0);
  const tmp = Buffer.allocUnsafe(64 * 1024);
  for (;;) {
    const got = await new Promise<number>((resolve, reject) =>
      read(fd, tmp, 0, tmp.length, null, (err, n) => (err ? reject(err) : resolve(n))),
    );
    if (got === 0) break; // EOF: Chrome closed the pipe
    ...
    for (;;) {
      ...
      if (ws && ws.readyState === WebSocket.OPEN) {
        ...
        ).catch((e) => log(`ws send failed: ${(e as Error).message}`));
      }
...
function pumpWsToStdin(ws: WebSocket): void {
  ...
  ws.on('message', (data: Buffer) => {
    chain = chain
      ...
      .catch((e) => {
        log(`outbound write failed: ${(e as Error).message}`);
        try { ws.close(NM_CLOSE_FRAME_PROTOCOL, 'frame_protocol_error'); } catch { /* noop */ }
      });
  });
}
...
function connectOnce(cfg: { wsUrl: string; token: string }): Promise<WebSocket> {
  return new Promise((resolve, reject) => {
...
async function connectWithRetry(cfg: { wsUrl: string; token: string }): Promise<WebSocket> {
  ...
  for (;;) {
    try {
      ...
      if (e instanceof HandshakePermanentError) throw e;
      const remaining = deadline - Date.now();
      if (remaining <= 0) throw e;
      const wait = Math.min(delay, remaining);
      log(`connect failed (${(e as Error).message}), retrying in ${Math.round(wait)}ms`);
...
async function main(): Promise<number> {
  ...
  if (PROBE) {
    ...
    if (frame) {
      ...
      frame.copy(out, 4);
      let off = 0;
      while (off < out.length) off += writeSync(1, out, off, out.length - off);
    }
    return 0;
  }
  ...
  for (;;) {
    ...
    try {
      ...
      log(`bridge connect failed permanently: ${(e as Error).message}`);
