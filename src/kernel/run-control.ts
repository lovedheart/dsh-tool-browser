 ...
 */
export function raceAbort(op: Promise<unknown>, signal: AbortSignal | undefined): Promise<unknown> {
  if (!signal) return op as Promise<unknown>;
 ...
 */
export function abortBrowserError(err: unknown): BrowserError {
  return new BrowserError({
    ...
    suggested_action:
      'Your browser session is still alive — open pages, state, and the `browser`/`page` ' +
